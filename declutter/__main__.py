...
def setParams(args):
    ...
    if args.show  and (args.maxdist is not None or args.maxtime is not None or
                       args.steps is not None or args.dx is not None
                       or args.dt is not None):
    ...
    if (args.steps is not None and
        ...
        elif args.steps is None or (args.maxdist is None
                                    and args.maxtime is None): 
            if args.steps is not None:
                parameters.setSteps(args.steps)
...
if __name__ == "__main__":
    ...
    subparsers = parser.add_subparsers(description="Select one of the commands "
    ...
    loadParse = subparsers.add_parser('load', description="Takes a raster and "
        "generates the slope and aspect of the surface, saving them along with "
        "the original raster in 'maps.hdf5'. The raster will be projected if a "
        "coordinate system other than UTM is used and the raster can also be "
        "changed to use a cell size which is a multiple of the original.",
                        help='Loads a raster into numpy arrays.')
    loadParse.add_argument("filename",help="Name of the raster file to load.")
    loadParse.add_argument("latitude",type=float,help="Latitude of the region, "
                           "used to determine UTM zone for projection.")
    loadParse.add_argument("longitude",type=float,help="Longitude of the "
                           "region, used to determine UTM zone for projection.")
    loadParse.add_argument("-c","--cellSize",type=float,
                           help="Allows the raster to be stored with a larger "
                           "cell size than the original.")
    loadParse.add_argument("-o","--out",help="Specifies a directory to store "
                           " the raster in if it is resampled or projected.")
    loadParse.set_defaults(func=load)

    # processing a path file to create a radargram
    modelParse = subparsers.add_parser('model',
        description="Assuming 'maps.hdf5' exists in the current directory, this "
                    "takes a series of points and generates an estimate of the "
                    "radargram expected.",
        help='Displays the radargram for a path.')
    modelParse.add_argument("filename",help="Name of the path file to load.")
    modelParse.add_argument("-t","--type",choices=["gpx","dst","xyz"],
        help="Specifies the file type, one of 'gpx', 'dst' or 'xyz'. "
             "This is identified by the file extension if not given.")
    modelParse.add_argument("--start",type = int,default=0,
        help = "How many points to ignore from the start of the path.")
    modelParse.add_argument("--end",type = int,default=0,
        help = "How many points to ignore from the end of the path.")
    modelParse.add_argument("-f","--files",action="store_true",
        help = "Store the intermediate data generated for each point.")
    modelParse.add_argument("-o","--out", #  only valid if -f selected
        help = "Specifies the directory to store the intermediate point "
                "data in.") 
    exclusive = modelParse.add_mutually_exclusive_group()
    modelParse.add_argument("-v","--view",action="store_true",# only valid if -f
        help = "Location of visible points stored too, helpful for analysis.") 
    exclusive.add_argument("-n","--no",action="store_false",
        help = "Do not save the radargram produced, but still displays it.")
    exclusive.add_argument("-s","--save",
    ...
    displayParse = subparsers.add_parser('display',
    ...
    displayParse.add_argument('directory',
        help="Name of the directory containing the point data.")
    displayParse.add_argument('-a','--adjusted',action="store_true",
