...
def viewshed(grid, pointx, pointy, mask, elevation=100, gridsize=30.0,
             stepSize=None):
    """Calculates which points on a grid are visible from a given viewpoint.
    If the ray from the viewpoint to a surface passes over a part of the grid
    where the height is unknown (NaN), that surface is treated as not being
    visible. This should only affect the borders of the grid.
    This also applies to points outside the grid.

    Parameters
    grid - 2D float array : A heightmap of the surface.
    pointx, pointy - floats : Coordinates of the viewpoint in cell size units.
    mask - 2D bool array : A mask of the points to consider. Everything else
        is assumed to be not visible. e.g. because those points are known to
        be out of range or have back facing surfaces.
    elevation - float (optional) : The elevation of the viewpoint. By default
        this is 100m.
    gridsize - float (optional) : The resolution of the grid. Default is 30m.
        Only important if stepSize is set.
    stepSize - float (optional) : The smallest resolution steps to take along
        the ray to each surface point to determine if it is visible.
        This is in world coordinates i.e. stepSize = 30 and gridSize = 30
        means the smallest step is 1 cell at a time.

    Returns
    view - 2D bool array : A mask where only points with value True are
        visible.
    """
    ...
    for s in range(scaleUp, -1, -1):
        result = _visible(grid, pointx, pointy, xs[m], ys[m], elevation,
                          stepSize*8.0**s).astype(bool)
