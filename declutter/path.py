...
def _makeDirections(xs, ys):
    ...
    direction[-1] = 180.0/np.pi*(np.arctan2(xs[-2]-xs[-1],
                                            ys[-2]-ys[-1]))+180.0
...
def _genPath(xs, ys, zs, name, save_visible=True, parallel=True):
    ...
    data = [(x, y, z, angle, save_visible, name+"/point"+str(i),
             parameters.env) for x, y, z, angle, i in zip(xs, ys, zs,
                                                          direction,
                                                          np.arange(steps))]
    fail = False
    try:
...
def _loadGpx(filename, crop=[0, 0], outName=None):
    ...
    lats, lons, zs = [], [], []
    for pt in root.iter(prefix+"trkpt"):  # searches points in document order
...
def gpsToXY(lons, lats):
    ...
    Although this will use UPS coordinates when outside the bounds of UTM
    zones, this has not been tested. In particular, elevation is unchanged.
    If UPS is ever needed, the output should be checked to see if elevation
    also needs some form of mapping or not to match the maps.hdf5 raster."""
...
def showAboveGround(filename, crop=[0, 0], style=None, offset=0):
    ...
    xs = (xs-left)/cellSize
    ys = (ys-low)/cellSize
    groundHeights = viewshed.quadHeight(grid, xs, ys)  # ground beneath points
...
def showOnSurface(filename, crop=[0, 0], extend=10, style=None, offset=0):
    ...
    xcoords = (max(0, int(xcoords[0]-extend)),
               min(width, int(xcoords[1]+extend)))
    ycoords = (np.amin(ys)-low)/cellSize, (np.amax(ys)-low)/cellSize
    ycoords = (max(0, int(ycoords[0]-extend)),
               min(height, int(ycoords[1]+extend)))
...
def checkValid(filename, crop=[0, 0], style=None):
    ...
    for i in range(len(xs)):
        ...
        if (np.any(np.isnan(heightmap[m])) or x < xBounds[0] or
                x > xBounds[1] or y < yBounds[0] or y > yBounds[1]):
            notFullRange += 1
