...
def processSlice(distance, angle, theta, phi, intensityModel=None,
    ...
    theta, phi - float arrays : the direction to each visible point in
        ...
        These can be None, in which case they are not considered.
    ...
    sample = np.array([np.sum(intensity[t == i])
                       for i in range(env.getSteps())])
...
def manyWiggle(name, adjusted=False, intensityModel=None,
    ...
    name - string : directory to look in for point data. Must contain no other
        files or directories.
    adjusted - bool (optional) : the data is shifted for each point to align
        the response from the surface directly beneath the radar with the top
        of the plot.
    intensityModel - function (optional) : a function of incidence angle for
        the backscatter from a surface.
    ...
    for i in range(len(files)):
        ...
        if compareTo is None:
            plt.fill_betweenx(ys, m*i, m*i+draw[i], where=(draw[i] > 0),
                              color='k')
        else:
    ...
    # correlation between actual and comparison data
