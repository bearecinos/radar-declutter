...
def _getDirections(vis,pointx,pointy,cropLeft,cropLow):
    ...
    directions[vis==1] = (180.0/math.pi * np.arctan2(pointx-cropLeft-_CellSize*x[vis==1],
...
def _makeIncidence(vis,elevation,heightmap,trueDist,distances,slope,aspect,directions):
    ...
    cosAng = (cosTheta * np.cos(math.pi/180.0*slope[m]) - sinTheta * np.sin(np.pi/180.0*slope[m]) *
...
def _makeAntenna(vis,heightmap,elevation,trueDist,directions,distances,antennaDir):
    """Generate theta and phi for a spherical coordinate system with the antenna along the z-axis.
    Result is in radians."""
    m = (vis==1)
    worldTheta = np.arccos((heightmap[m]-elevation)/trueDist[m]) # angle from vertical down to point
    worldPhi = ((directions[m]-antennaDir)%360)*math.pi/180.0 # bearing difference converted to radians
...
def generateMaps(pointx,pointy,elevation=100.0,antennaDir=None):
    ...
    # avoid generating data where not needed
