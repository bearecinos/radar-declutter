...
def _getDirections(vis, pointx, pointy, cropLeft, cropLow):
    ...
    directions[vis == 1] = (180.0/math.pi *
                            np.arctan2(pointx-cropLeft-_CellSize*x[vis == 1],
                                       pointy-cropLow-y[vis == 1]*_CellSize) +
                            180)
    return directions
...
def _makeDistance(vis, distances, heightmap, elevation):
    ...
    trueDist[vis == 1] = np.hypot(distances[vis == 1],
                                  heightmap[vis == 1]-elevation)
    return trueDist
...
def generateMaps(pointx, pointy, elevation=100.0, antennaDir=None):
    ...
    incidence - float array : the surface incidence angle (in radians) of every
        visible point.
    theta, phi - float arrays : the direction to each visible point in
        spherical coordinates, with the ends of the antenna being the poles.
        These will be None if antennaDir was not given.
    ...
    if np.isnan(groundHeight):  # Assumes use of NaN for noData, can't use ==.
        ...
        return (np.full(heightmap.shape, 0, int), (cropLeft, cropLow),
                np.empty(0), np.empty(0), theta, phi, elevation)
    ...
    vis = viewshed.viewshed(heightmap, (pointx-cropLeft)/_CellSize,
                            ...
                            elevation, gridsize=_CellSize)  # bool array

    if antennaDir is not None:
        theta, phi = _makeAntenna(vis, heightmap, elevation, trueDist,
                                  directions, distances, antennaDir)
...
def store(path, dist, incidence, x, y, elevation, vis=None, visCorner=None,
    ...
    theta, phi - float arrays : the direction to each visible point in
        spherical coordinates, with the ends of the antenna being the poles.
        By default these are None. These should be None iff antennaDir is None.
    ...
    try:
        with h5py.File(path, "w") as f:
            ...
            if antennaDir is not None:
                f.create_dataset("antennaTheta", compression="gzip",
                                 data=theta)
